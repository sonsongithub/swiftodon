...
class StatusViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let list = MastodonSession.sessions()
        if list.count > 0 {
            statusController = StatusController(session: list[0])
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        guard let controller = statusController else { return 0 }
        return controller.contents.count
    }
    
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        guard let controller = statusController else { return 0 }
        return controller.contents[indexPath.row].height + 84
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TimelineCell", for: indexPath)

        if let controller = statusController, let cell = cell as? TimelineCell {
